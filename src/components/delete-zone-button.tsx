...
export function DeleteZoneButton({
  ...
  async function onDelete() {
    if (
      !window.confirm(
        `Delete zone leader "${username}"? This permanently removes it and cannot be undone.`,
      )
    ) {
      return;
    }
    setBusy(true);
    try {
      const res = await fetch(`/api/zone-leaders/${id}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok || !data.ok) {
        toast.error(data.error || "Delete failed");
        return;
      }
      toast.success(`Zone leader "${username}" deleted`);
      router.refresh();
    } catch {
